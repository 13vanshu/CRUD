...
const Home = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const url = isUpdating
                ? `http://localhost:2000/books/booklists/${updateId}`
    ...
    const fetchBooks = async () => {
        try {
            const res = await fetch("http://localhost:2000/books/booklists");
            const data = await res.json();
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const res = await fetch(`http://localhost:2000/books/booklists/${id}`, {
                method: "DELETE",
            });
